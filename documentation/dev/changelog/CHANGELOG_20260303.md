...
### 问题
`--stop-server` 命令存在两个问题：
1. 执行后会启动 MCP 服务器等待连接，而不是立即退出
2. 强制要求 USB 连接，但驻留服务端只在网络模式设计

### 修复
| 文件 | 修改 |
|------|------|
| `scrcpy_http_mcp_server.py` | 重构 `--stop-server` 逻辑 |

### 变更
1. **命令行为**：执行完成后立即退出，不再启动 MCP 服务器
2. **支持网络模式**：通过 UDP 协议发送 `SCRCPY_TERMINATE` 消息
3. **参数支持**：
   - `--stop-server` - 自动通过 USB 获取设备 IP
   - `--stop-server <IP>` - 直接发送终止请求到指定 IP
...
# 直接指定 IP
python scrcpy_http_mcp_server.py --stop-server 192.168.5.4
```

### 技术细节
- UDP 终止请求: `SCRCPY_TERMINATE` → 端口 27183
- 期望响应: `SCRCPY_TERMINATE_ACK`
- UDP 广播发现: `SCRCPY_DISCOVER` → 端口 27183
- 期望响应: `SCRCPY_HERE DeviceName IP mode`
- 如果 USB 不可用， 尝试通过 USB 获取设备 IP
- 如果 USB 也失败, 尝试 UDP 广播发现驻留服务端
- 与 Companion 应用使用相同协议
